...
From the plots, we can see the overall observations in experimental group are higher than the control group and the observation for each day in a week remains at the same level for control group, wheras increases for observation group.
...
```
**Describe the association between effect size and power:** Power increases with the increase of size effect.
...
all_info = left_join(sim_all, beta1_true_average) %>% 
  left_join(beta1_reject_average)
  

all_info %>% 
  ggplot(aes(x = beta1_true)) + geom_point(aes(y = average), size = 5, color = "pink") +
...
Because the null here is beta1 = 0. When we reject the null, we get result from beta1$\neq$0, so the model changes here.
